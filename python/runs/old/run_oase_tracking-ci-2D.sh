...
if [ "X$1" = "X" ]; then
    echo "run_tracking.sh <path to directory containing composite files>  <scale> [resume]"
...
cat $MEANIE3D_HOME/visit/tracking/run_oase_tracking-ci-2D.py | sed -e "s/PARAM_SOURCE_DIR/$ESCAPED_SOURCE_DIR/g" | sed -e "s/PARAM_DL_PATH/$ESCAPED_DL_PATH/g" | sed -e "s/M3D_HOME/$ESCAPED_MEANIE3D_HOME/g" | sed -e "s/PARAM_SCALE/$2/g" | sed -e "s/P_RESUME/$resume/g" > $SCRIPTFILE
