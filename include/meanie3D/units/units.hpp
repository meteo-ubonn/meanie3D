...
#define UNIT_DISPLAY_NAME(unit, string)              \
    template <>                                      \
    struct output_unit<unit>                         \
    {                                                \
        template <typename Stream>                   \
        static void fn(Stream &os) { os << string; } \
    };

#define UNITS_DISPLAY_NAME(unit, string) \
    namespace units                      \
    {                                    \
        UNIT_DISPLAY_NAME(unit, string)  \
    }
...
namespace units
{
    ...
    UNIT_DISPLAY_NAME(units::byte, "bytes");
    ...
    namespace units
    {
        ...
        // binary SI prefixes (capital letter):
        template <typename Unit>
        struct Kilo
        {
            typedef scale<Unit, 1, 1024> type;
        };
        template <typename Unit>
        struct Mega
        {
            typedef scale<typename Kilo<Unit>::type, 1, 1024> type;
        };
        template <typename Unit>
        struct Giga
        {
            typedef scale<typename Mega<Unit>::type, 1, 1024> type;
        };
        template <typename Unit>
        struct Tera
        {
            typedef scale<typename Giga<Unit>::type, 1, 1024> type;
        };
        template <typename Unit>
        struct Peta
        {
            typedef scale<typename Tera<Unit>::type, 1, 1024> type;
        };

        typedef Kilo<byte>::type KiB;
        typedef Mega<byte>::type MiB;
        typedef Giga<byte>::type GiB;
        typedef Tera<byte>::type TiB;
        typedef Peta<byte>::type PiB;
    } // namespace units
    ...
    UNIT_DISPLAY_NAME(units::KiB, "KiB");
    UNIT_DISPLAY_NAME(units::MiB, "MiB");
    UNIT_DISPLAY_NAME(units::GiB, "GiB");
    UNIT_DISPLAY_NAME(units::TiB, "TiB");
    UNIT_DISPLAY_NAME(units::PiB, "PiB");
    ...
    namespace units
    {
        ...
        // Non-SI velocity (transfer/bandwidth)
        typedef compose<byte, pow<s, -1> >  bps;
        typedef compose<KiB, pow<s, -1> >  KiBps;
        typedef compose<MiB, pow<s, -1> >  MiBps;
        typedef compose<GiB, pow<s, -1> >  GiBps;
        typedef compose<TiB, pow<s, -1> >  TiBps;
        typedef compose<PiB, pow<s, -1> >  PiBps;
    ...
    UNIT_DISPLAY_NAME(units::bps, "bytes/s");
    UNIT_DISPLAY_NAME(units::KiBps, "KiB/s");
    UNIT_DISPLAY_NAME(units::MiBps, "MiB/s");
    UNIT_DISPLAY_NAME(units::GiBps, "GiB/s");
    UNIT_DISPLAY_NAME(units::TiBps, "TiB/s");
    UNIT_DISPLAY_NAME(units::PiBps, "PiB/s");
    ...
    namespace values
    {
        ...
        // SI binary
        typedef value<unsigned long long, units::byte> byte;
        typedef value<unsigned long long, units::KiB> KiB;
        typedef value<unsigned long long, units::MiB> MiB;
        typedef value<unsigned long long, units::GiB> GiB;
        typedef value<unsigned long long, units::TiB> TiB;
        typedef value<unsigned long long, units::PiB> PiB;
        ...
        typedef value<double, units::KiBps> KiBps;
        typedef value<double, units::MiBps> MiBps;
        typedef value<double, units::GiBps> GiBps;
        typedef value<double, units::TiBps> TiBps;
        typedef value<double, units::PiBps> PiBps;
