...
if [ "X$1" = "X" ]
then
    echo "visualize_clusters.sh <netcdf directory> <cluster directory> #slices"
...
cat $MEANIE3D_HOME/visit/visualize_results/visualize_rico_clusters_2D.py | sed -e "s/P_NETCDF_FILE/$ESCAPED_NETCDF_FILE/g" | sed -e "s/P_CLUSTER_DIR/$ESCAPED_CLUSTER_DIR/g" | sed -e "s/P_M3D_HOME/$ESCAPED_MEANIE3D_HOME/g" | sed -e "s/P_SLICES/$3/g" > ${SCRIPTFILE}
