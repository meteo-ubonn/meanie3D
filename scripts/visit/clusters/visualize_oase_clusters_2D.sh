...
cat $MEANIE3D_HOME/scripts/visit/clusters/visualize_oase_clusters_2D.py | sed -e "s/P_NETCDF_DIR/$ESCAPED_NETCDF_DIR/g" | sed -e "s/P_CLUSTER_DIR/$ESCAPED_CLUSTER_DIR/g" | sed -e "s/P_M3D_HOME/$ESCAPED_MEANIE3D_HOME/g" | sed -e "s/P_OVERLAY/$OVERLAY/g" | sed -e "s/P_RESUME_AT_FILE/${ESCAPED_RESUME_FILE}/g" > ${SCRIPTFILE}
